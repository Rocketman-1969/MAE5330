...
def derivatives(state: types.DynamicState, forces_moments: types.ForceMoment) -> types.DynamicState:
    ...
    ned=Quaternion2Rotation(np.array([[st.e0],[st.e1],[st.e2],[st.e3]]))@np.array([[st.u],[st.v],[st.w]])
    
    # collect the derivative of the states
    x_dot = np.empty( (IND.NUM_STATES,1) )
    x_dot[IND.NORTH] = ned[0,0]
    x_dot[IND.EAST] = ned[1,0]
    x_dot[IND.DOWN] = ned[2,0]
    ...
    x_dot[IND.E0] = .5*(st.e0*0+st.e1*-st.p+st.e2*-st.q+st.e3*-st.r)
    x_dot[IND.E1] = .5*(st.e0*st.p+0*st.e1+st.e2*st.r+st.e3*-st.q)
    ...
    x_dot[IND.Q] = (MAV.gamma5*st.p*st.r)-(MAV.gamma6*(st.p**2-st.r**2))+(1/MAV.Jy*fm.m)
