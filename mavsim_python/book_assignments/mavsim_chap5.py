...
def main() -> None:
    ...
    sim_params = MsgSimParams(end_time=100., write_video = True, video_name="chap5.avi") # Sim ending in 10 seconds
    state = DynamicState()
    ...
    gamma_trim = 0.1
    radius_trim = np.inf
    trim_state, trim_input = compute_trim(state0=state.convert_to_numpy(), Va=Va_trim, gamma=gamma_trim, R=radius_trim)
